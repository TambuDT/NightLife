...
export default function Navbar() {
  return (
    <Tab.Navigator screenOptions={{ headerShown: false, tabBarShowLabel: false, tabBarStyle: { backgroundColor: '#1c1c1c',borderTopWidth: 0 }, tabBarActiveTintColor: '#fff'}}>
      <Tab.Screen name='Explore' component={Explore} options={{ tabBarIcon: ({ color, size }) => (<Ionicon name='location-outline' color={color} size={size} />) }}></Tab.Screen>
      <Tab.Screen name='Search' component={Search} options={{ tabBarIcon: ({ color, size }) => (<Ionicon name='compass-outline' color={color} size={size} />) }}></Tab.Screen>
